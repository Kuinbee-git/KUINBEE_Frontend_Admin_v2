...
export function SupplierAnalytics({ supplierId }: SupplierAnalyticsProps) {
  ...
  const analyticsQuery = useSupplierAnalytics(supplierId, { windowDays });
  const { data: supplier, isLoading: isLoadingSupplier } = supplierQuery;
  const { data: analytics, isLoading: isLoadingAnalytics } = analyticsQuery;
  ...
  if (supplierQuery.isError || analyticsQuery.isError) {
    const error = supplierQuery.error ?? analyticsQuery.error;
    return (
      <div className="flex min-h-[60vh] items-center justify-center bg-[var(--bg-surface)] p-4">
        <Card className="w-full max-w-lg">
          <CardContent className="py-10 text-center">
            ...
            <h1 className="mt-3 text-xl">Could not load supplier analytics</h1>
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--bg-surface)' }}>
      ...
      >
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            <SelectTrigger className="w-full sm:w-45" aria-label="Analytics time window">
              ...
              >
                ...
                <p className="text-2xl font-bold mt-1" style={{ color: 'var(--text-primary)' }}>
                  {analytics.totals.datasetCount > 0
                    ? Math.round(
                        (analytics.totals.publishedDatasetCount / analytics.totals.datasetCount) *
                          100
                      )
                    : 0}
                  %
                ...
                <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
                  {analytics.totals.publishedDatasetCount} of {analytics.totals.datasetCount}{' '}
                  datasets published
                </p>
              </div>
              ...
              >
                ...
                <p className="text-2xl font-bold mt-1" style={{ color: 'var(--text-primary)' }}>
                  {analytics.totals.orderCount > 0
                    ? formatCurrency(
                        (
                          parseFloat(analytics.totals.revenue) / analytics.totals.orderCount
                        ).toString()
                      )
                    : formatCurrency('0')}
                </p>
              ...
              >
                ...
                <p className="text-2xl font-bold mt-1" style={{ color: 'var(--text-primary)' }}>
                  {analytics.totals.publishedDatasetCount > 0
                    ? formatCurrency(
                        (
                          parseFloat(analytics.totals.revenue) /
                          analytics.totals.publishedDatasetCount
                        ).toString()
                      )
                    : formatCurrency('0')}
                </p>
